		...
		struct _FileWrapper
		{
			...
			static long		ZCALLBACK seek_file_func	(voidpf opaque, voidpf stream, uLong offset, int origin)
			{
				...
				switch ( origin )
				{
					...
					default :			RETURN_ERR( "invalid seek falg", 0 );
				}
...
namespace GXTypes
{
	typedef SHARED_POINTER_TYPE( File::ZipRFile )	ZipRFilePtr;
	typedef SHARED_POINTER_TYPE( File::ZipWFile )	ZipWFilePtr;
