...
namespace GXTypes
{
	...
	public:
		TString (GX_DEFCTOR);
		TString (Self &&str);
		TString (const Self &str);
		TString (TStringRef<const T> str);
		...
		Bytes<usize>	FullSize ()							const		{ return Bytes<usize>( _size * sizeof(T) ); }
		...
		operator		Buffer<T> ()									{ return Buffer<T>( _memory.Pointer(), Length() ); }
		operator		Buffer<const T> () const						{ return Buffer<const T>( _memory.Pointer(), Length() ); }
	...
	template <typename T, typename S, typename MC>
	inline TString<T,S,MC>::TString (Uninitialized) : _length(0), _size(0)
	{}
	...
	{
		Copy( TStringRef<const T>( str, length == 0 and str != null ? StringUtils::Length( str ) : length ) );
	}
		
	
	template <typename T, typename S, typename MC>
	inline TString<T,S,MC>::TString (const Self &str) : _length(0), _size(0)
	{
		Copy( TStringRef<const T>( str ) );
	}
	

	template <typename T, typename S, typename MC>
	inline TString<T,S,MC>::TString (Self &&str) :
		_memory( RVREF( str._memory ) ),
		_length( str._length ),
		_size( str._size )
	{
		str._length	= 0;
	...
	{
		FormatI( value, CoreTypesConfig::StringDefaultRadix );
	}
	...
	inline void TString<T,S,MC>::Resize (usize length, T value, bool allowReserve)
	{
		...
		const isize	i_delta = (isize)( length - Length() );
	...
	inline TString<T,S,MC> & TString<T,S,MC>::FormatI (const B& value, int radix)
	{
		...
		_length = StringUtils::IntToStr( value, _memory.Pointer(), _size, radix );
		return *this;
	}
	...
	inline TString<T,S,MC> & TString<T,S,MC>::FormatI (const B& value, usize stepSize, T spaceChar, int radix)
	{
		...
		isize	i	= StringUtils::IntToStr( value, a_buf, CountOf(a_buf), radix ),
				j	= 0,
