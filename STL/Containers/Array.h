...
namespace GXTypes
{
	...
	public:
		...
		operator		Buffer<T> ()									{ return Buffer<T>( _memory.Pointer(), _count ); }
		operator		Buffer<const T> () const						{ return Buffer<const T>( _memory.Pointer(), _count ); }
		...
		//void Replace (Buffer<const T> other, usize pos);
		//void Replace (Buffer<T> other, usize pos);
		//void Replace (const Self &other, usize pos)				{ return Replace( (Buffer<const T>) other, pos ); }
	...
	inline Array<T,S,MC>::Array (Uninitialized): _count(0), _size(0)
	{}
