...
namespace GXTypes
{
	...
	public:
		...
		CircularQueue (Buffer<const T> other);
		...
		bool			operator == (Buffer<const T> right)	const;
		...
		void AppendFront (Buffer<const T> value);
		void AppendFront (const Self &value);
		void AppendFront (Self &&value);

		void AppendBack (Buffer<const T> value);
		...
		void GetParts (OUT Buffer<T> &part0, OUT Buffer<T> &part1);
		void GetParts (OUT Buffer<const T> &part0, OUT Buffer<const T> &part1) const;

		usize			Count ()		const;
		Bytes<usize>	Size ()			const		{ return Bytes<usize>( sizeof(T) * Count() ); }
		...
		Bytes<usize>	FullSize ()		const		{ return Bytes<usize>( sizeof(T) * Capacity() ); }
		bool			Empty ()		const		{ return _first == _end; }
		usize			LastIndex ()	const		{ return Count()-1; }
		

		static constexpr bool	IsLinearMemory ()	{ return true; }	// TODO: false ?
	...
	inline CircularQueue<T,S,MC>::CircularQueue (Uninitialized) :
	...
	{
		Buffer<const T>	part0;
		Buffer<const T>	part1;
	...
	inline void CircularQueue<T,S,MC>::GetParts (Buffer<T> &part0, Buffer<T> &part1)
	{
		...
		if ( _first > _end )
		{	
			part0 = Buffer<T>( _memory.Pointer() + _first, _size - _first );
			part1 = Buffer<T>( _memory.Pointer(), _end );
		}
		else
		{
			part0 = Buffer<T>( _memory.Pointer() + _first, Count() );
			part1 = Buffer<T>();
		}
	}
	...
	inline void CircularQueue<T,S,MC>::GetParts (Buffer<const T> &part0, Buffer<const T> &part1) const
	{
		if ( Empty() )
		{
			part0 = Buffer<const T>();
			part1 = Buffer<const T>();
		}
		else
		if ( _first > _end )
		{	
			part0 = Buffer<const T>( _memory.Pointer() + _first, _size - _first );
			part1 = Buffer<const T>( _memory.Pointer(), _end );
		}
		else
		{
			part0 = Buffer<const T>( _memory.Pointer() + _first, Count() );
			part1 = Buffer<const T>();
		}
	}
