...
namespace GXTypes
{
	...
	public:
		...
		operator	Buffer<T> ()										{ return Buffer<T>( _memory.Pointer() + _first, Count() ); }
		operator	Buffer<const T> () const							{ return Buffer<const T>( _memory.Pointer() + _first, Count() ); }
	...
	inline Queue<T,S,MC>::Queue (Uninitialized) : _size(0), _first(0), _last(-1) {}
	...
	{
		const usize	nom	= CoreTypesConfig::MemContainerResizingNominator;
		const usize	den	= CoreTypesConfig::MemContainerResizingDenominator;
	...
	inline void Queue<T,S,MC>::_ReplaceToLeft (usize offset)
	{
		...
		const isize	min_size	= CoreTypesConfig::MemContainerResizingMinSize;
