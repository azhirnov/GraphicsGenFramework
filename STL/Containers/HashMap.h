...
namespace _types_hidden_
{
	...
	public:
		...
		usize Add (const key_t &key, const value_t &value)
		{
			...
			return _memory.Add( RVREF(triple_t( hash, key, value )) );
		}

		usize Add (key_t &&key, value_t &&value)
		{
			const key_hash_t	hash = _hash( key );
			return _memory.Add( RVREF(triple_t( hash, RVREF(pair_t( RVREF(key), RVREF(value) )) )) );
		}

		usize Add (const pair_t &value)
		{
			const key_hash_t	hash = _hash( value.first );
			return _memory.Add( RVREF(triple_t( hash, RVREF(pair_t( value.first, value.second )) )) );
		}

		usize Add (pair_t &&value)
		{
			const key_hash_t	hash = _hash( value.first );
			return _memory.Add( RVREF(triple_t( hash, RVREF(value) )) );
		}


		void AddArray (Buffer<const pair_t> value)
		{
			FOR( i, value ) {
				Add( value[i] );
			}
	...
	struct Hash< _types_hidden_::BaseHashMap< K, T, IsUnique, H, S, MC > > :
		private Hash< Buffer<const Pair< typename H::result_t, Pair<K, T> > > >
	{
		typedef _types_hidden_::BaseHashMap< K, T, IsUnique, H, S, MC >				key_t;
		typedef Hash< Buffer<const Pair< typename H::result_t, Pair<K, T> > > >		base_t;
