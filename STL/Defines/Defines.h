...
#include "AuxiliaryDefines.h"


#ifndef GX_EXPORT
# ifdef __GX_BUILD_STATIC_LIB__
#	define GX_EXPORT
# else
#	ifdef __GX_BUILD__
...
#if defined( __GX_DEBUG__ )
# ifndef ASSERT_EXT
#	define ASSERT_EXT( _condition_, _message_ )		assert2( _condition_, _message_ )
# endif
# define ASSUME( _condition_ )						ASSERT( _condition_ )
#else
# define ASSUME( _condition_ )						GX_ASSUME( _condition_ )
# ifndef ASSERT_EXT
#	define ASSERT_EXT( _condition_, _message_ )		
# endif
#endif	// __GX_DEBUG__

#define ASSERT( _condition_ )				ASSERT_EXT( (_condition_), TOSTRING(_condition_) )



// Static Assert //
#define STATIC_ASSERT( ... ) \
	static_assert(	AUXDEF_GETRAW( AUXDEF_GET_FIRST( __VA_ARGS__ ) ), \
					AUXDEF_GETRAW( AUXDEF_GET_SECOND( __VA_ARGS__, TOSTRING(__VA_ARGS__) ) ) )

#define TODO( _message_ )					ASSERT_EXT( false, _message_ )
#define WARNING( _message_ )				ASSERT_EXT( false, _message_ )
...
// try / catch //
#if defined( GX_CATCHING_EXCEPTIONS_DISABLE )
#	define TRY								
#	define THROW( _val_ )
#	define CATCH( _val_, ... )
#else
...
#endif	// GX_CATCHING_EXCEPTIONS_DISABLE


// Other //

// compile-time count of array
#if defined( GX_CONSTEXPR_SUPPORTED )
#	define COUNT_OF( _arr_ )				::GX_STL::GXTypes::CountOf( _arr_ )
#else
#	define COUNT_OF( _arr_ )				(sizeof(_arr_) / sizeof(_arr_[0]))
#endif

// compile-time constant
//#ifdef GX_CONSTEXPR_SUPPORTED
//#	define GX_CONST							constexpr
//#else
...
// Short file name
#define GX_SHORT_FILENAME		(::GX_STL::GXTypes::FileAddressUtils::ToShortPath( __FILE__, 2 ))
...
#define GX_DEFCTOR		::GX_STL::GXTypes::Uninitialized = ::GX_STL::GXTypes::Uninitialized()
