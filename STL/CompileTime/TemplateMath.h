...
namespace CompileTime
{
	...
	namespace _ctime_hidden_
	{
		...
		{
			STATIC_ASSERT( Value1 != T(0) );
			static const T	value = _GreatestCommonDivisor< T, Value1, Value0 % Value1 >::value;
		};

		// TODO: delete this shit
		template <byte Value0>
		struct _GreatestCommonDivisor < byte, Value0, 0 > {
			static const byte	value = Value0;
		};

		template <ubyte Value0>
		struct _GreatestCommonDivisor < ubyte, Value0, 0 > {
			static const ubyte	value = Value0;
		};

		template <short Value0>
		struct _GreatestCommonDivisor < short, Value0, 0 > {
			static const short	value = Value0;
		};

		template <ushort Value0>
		struct _GreatestCommonDivisor < ushort, Value0, 0 > {
			static const ushort	value = Value0;
		};

		template <int Value0>
		struct _GreatestCommonDivisor < int, Value0, 0 > {
			static const int	value = Value0;
		};

		template <uint Value0>
		struct _GreatestCommonDivisor < uint, Value0, 0 > {
			static const uint	value = Value0;
		};

		template <ilong Value0>
		struct _GreatestCommonDivisor < ilong, Value0, 0 > {
			static const ilong	value = Value0;
		};

		template <ulong Value0>
		struct _GreatestCommonDivisor < ulong, Value0, 0 > {
			static const ulong	value = Value0;
		};
	...
	template <typename T, T Value, uint AlignTo>
	static constexpr T Align	= _ctime_hidden_::_Align< T, Value, AlignTo >::value;
