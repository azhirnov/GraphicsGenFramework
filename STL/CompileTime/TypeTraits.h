...
namespace GXTypes
{
	...
	namespace TypeTraits
	{
		...
		// RemoveQualifier
		template <typename T>
		using RemoveQualifier = typename _TypeQualifierInfo<T>::type;
		...
		// IsBaseType
		template <typename T>
		constexpr bool IsBaseType()		{ return _TypeQualifierInfo<T>::is_final; }
		...
		// GetBaseType
		template <typename T>
		using GetBaseType = typename _GetBaseType<T>::type;
		


		//--------- Get All Qualifiers --------//
		
		/*template <typename T, typename Info = _TypeQualifierInfo<T>, bool Continue = IsBaseType<T>()>
		struct _GetAllQualifiers
		{
			typedef _GetAllQualifiers< RemoveQualifier<T> >		NextInfo;

			static const bool	is_pointer		= NextInfo::is_pointer		| Info::is_pointer;
			static const bool	is_const		= NextInfo::is_const		| Info::is_const;
			static const bool	is_volatile		= NextInfo::is_volatile		| Info::is_volatile;
			static const bool	is_reference	= NextInfo::is_reference	| Info::is_reference;
			static const bool	is_rvalue_ref	= NextInfo::is_rvalue_ref	| Info::is_rvalue_ref;
			static const bool	is_array		= NextInfo::is_array		| Info::is_array;
		};

		template <typename T, typename Info>
		struct _GetAllQualifiers < T, Info, true > : Info	{};*/
		...
		// IsConstOrVolatile
		template <typename T>
		constexpr bool IsConstOrVolatile = _IsConstOrVolatile<T>::value;
		...
		// RemoveConstVolatile
		template <typename T>
		using RemoveConstVolatile = typename _RemoveConstVolatile<T>::type;
		...
		// IsPointer
		template <typename T>
		constexpr bool IsPointer = _TypeQualifierInfo< RemoveConstVolatile<T> >::is_pointer;
		...
		// IsReference
		template <typename T>
		constexpr bool IsReference			= _TypeQualifierInfo< RemoveConstVolatile<T> >::is_reference;

		// IsRValueReference
		template <typename T>
		constexpr bool IsRValueReference	= _TypeQualifierInfo< RemoveConstVolatile<T> >::is_rvalue_ref;
		...
		// RemoveAnyReference
		template <typename T>
		using RemoveAnyReference	= typename _RemoveAnyReference<T>::type;
		...
		// IsConst
		template <typename T>
		constexpr bool IsConst		= _TypeQualifierInfo<T>::is_const;

		// InvertConst
		template <typename T>
		using InvertConst			= typename _InvertConst<T>::type;

		// CopyConst
		template <typename FromType, typename ToType>
		using CopyConst				= typename _CopyConst< FromType, ToType >::type;
		
		// CopyConstToPointer
		template <typename FromType, typename ToType>
		using CopyConstToPointer	= typename _CopyConstToPointer< FromType, ToType >::type;
		
		// CopyConstToReference
		template <typename FromType, typename ToType>
		using CopyConstToReference	= typename _CopyConstToReference< FromType, ToType >::type;
		...
		// IsVolatile
		template <typename T>
		constexpr bool IsVolatile	= _TypeQualifierInfo<T>::is_volatile;

		// InvertVolatile
		template <typename T>
		using InvertVolatile		= typename _InvertVolatile<T>::type;

		// CopyVolatile
		template <typename T, typename B>
		using CopyVolatile			= typename _CopyVolatile<T,B>::type;
		...
		template <typename T>
		constexpr bool IsMemberVariablePointer	= _IsMemberVariablePointer< T >::value;
		...
		template <typename T>
		constexpr bool IsMemberFunctionPointer	= _IsMemberFunctionPointer< T >::value;
		...
		template <typename T>
		constexpr bool IsFunctionPointer	= _IsFunctionPointer< T >::value;
		...
		template <typename T>
		constexpr bool IsEnum	= _IsEnum< T >::value;
...
*/
	...
	// std::move
	template <typename T>
	inline typename RValueRef<T>::type  ToRValueRef (T&& arg)
	{
