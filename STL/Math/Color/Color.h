...
namespace GXMath
{
	...
	private:
		...
		struct LuminanceK {
			...
			static Vec<T,3>	Get()				{ return Vec<T,3>( 0.2126, 0.7152, 0.0722 ); }
		};
	...
	public:
		...
		explicit
		color4u (ubyte val): ubyte4( val ) {}

		explicit
		color4u (int val): ubyte4( ubyte(val) ) {}

		color4u (ubyte R, ubyte G, ubyte B, ubyte A = 255): ubyte4( R, G, B, A) {}

		template <typename T>
		color4u (const Vec<T,4> &v): ubyte4( ColorUtils::Convert<ubyte>( v ) ) {}
...
#			undef __DEF_COLOR
	...
	template <typename T>
	inline Vec<T,4>  ColorUtils::FromRGBA8 (uint x)
	{
	...
	template <>
	inline Vec<ubyte,4>  ColorUtils::FromRGBA8<ubyte> (uint x)
	{
	...
	template <typename T>
	inline uint  ColorUtils::ToRGBA8 (const Vec<T,4> &x)
	{
	...
	template <>
	inline uint  ColorUtils::ToRGBA8<ubyte> (const Vec<ubyte,4> &x)
	{
		uint	u_rgba = 0;
	...
	template <typename T>
	inline Matrix<T,4,4>  ColorUtils::SaturationMat (const T& s)
	{
	...
	inline float3  ColorUtils::RGBtoHSV (const float3 &rgb)
	{
	...
	inline float3  ColorUtils::HSVtoRGB (const float3 &hsv)
	{
	...
	inline float3  ColorUtils::RainbowRGB (const float value)
	{
