...
namespace GXMath
{
	...
	/*template <typename T>
	inline TFrustum<T> const & PerspectiveCamera<T>::Frustum ()
	{
		if ( _changed.Get(1) )
		{
	...
	inline void PerspectiveCamera<T>::_LimitRotation (OUT radians3_t &rotation) const
	{
		if ( _limitRotationEnabled )
		{
			...
			_rotationLimitMask.x ? 0.0f : rotation.x = Clamp( last_rot.x, _rotationMinAngle.x, _rotationMaxAngle.x ) - _rotation.x;
			_rotationLimitMask.y ? 0.0f : rotation.y = Clamp( last_rot.y, _rotationMinAngle.y, _rotationMaxAngle.y ) - _rotation.y;
			_rotationLimitMask.z ? 0.0f : rotation.z = Clamp( last_rot.z, _rotationMinAngle.z, _rotationMaxAngle.z ) - _rotation.z;
		}
	}
	...
	inline void PerspectiveCamera<T>::RotateFPS (const radians2_t &rotation)
	{
		...
		if ( IsNotZero( rot.x ) ) {
			q = quat_t().RotationY( rot.x ) * q;
			changed = true;
		}

		if ( IsNotZero( rot.y ) ) {
			q = q * quat_t().RotationX( rot.y );
			changed = true;
		}
