...
namespace GXTypes
{
	...
	public:
		...
		explicit constexpr operator int_t ()  const	{ return _value; }
		explicit constexpr operator uint_t () const	{ return _value; }

		friend void const*	operator + (void const *left, const Bytes<T> &right)
		{
			return (void const*)( ((byte const*) left) + _value );
		}
		
		friend void *		operator + (void *left, const Bytes<T> &right)
		{
			return (void*)( ((byte*) left) + _value );
		}
