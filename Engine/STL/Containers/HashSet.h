...
namespace _types_hidden_
{
	...
	public:
		...
		usize Add (const Value &value)
		{
			...
			return _memory.Add( RVREF(hpair_t( hash, value )) );
		}

		usize Add (Value &&value)
		{
			const key_hash_t	hash = _hash(value);
			return _memory.Add( RVREF(hpair_t( hash, RVREF(value) )) );
		}


		void AddArray (Buffer<const Value> value)
		{
			FOR( i, value ) {
				Add( value[i] );
			}
	...
	struct Hash< _types_hidden_::BaseHashSet< Value, IsUnique, H, S, MC > > :
		private Hash< Buffer<const Pair< typename H::result_t, Value > > >
	{
		typedef _types_hidden_::BaseHashSet< Value, IsUnique, H, S, MC >	key_t;
		typedef Hash< Buffer<const Pair< typename H::result_t, Value > > >	base_t;
