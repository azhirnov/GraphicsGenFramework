...
#	pragma warning (pop)
	...
	struct TypeInfo<void>
	{
		...
		static constexpr void	Max()		{ return void(); }
		static constexpr void	Min()		{ return void(); }
		static constexpr void	Inf()		{ return void(); }
		static constexpr void	NaN()		{ return void(); }
		
		static constexpr void	Epsilon()	{ return void(); }
		static constexpr uint	SignBit()	{ return 0; }
		static constexpr uint	Count()		{ return 0; }
	};
...
namespace GXTypes
{
	...
	{
		return ( GXMath::Abs(val) <= CompileTime::_ctime_hidden_::FLOAT_MAX or
				 GXMath::Abs(val) >= CompileTime::_ctime_hidden_::FLOAT_MIN );
	}

	template <>
	inline bool CheckValue<double>(const double& val)
	{
		return ( GXMath::Abs(val) <= CompileTime::_ctime_hidden_::DOUBLE_MAX or
				 GXMath::Abs(val) >= CompileTime::_ctime_hidden_::DOUBLE_MIN );
	}
	*/
