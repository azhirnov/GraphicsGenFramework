...
=================================================
*/
	bool ConditionVariable::Wait (CriticalSection &cs, uint time)
	{
		ASSERT( IsValid() );
		return _SleepConditionVariableCS(	&_cv.Get<CONDITION_VARIABLE>(),
											&cs._crSection.Get<CRITICAL_SECTION>(), time ) == TRUE;
	}
...
=================================================
*/
	bool SyncEvent::Wait (uint time)
	{
		return ::WaitForSingleObject( _event.Get<HANDLE>(), time ) == WAIT_OBJECT_0;
	}
...
=================================================
*/
	int SyncEvent::WaitEvents (Buffer<Self *> events, bool waitAll, uint time)
	{
		...
		DWORD	res		= ::WaitForMultipleObjects( (DWORD)events_arr.Count(), events_arr.ptr(), waitAll, time );
