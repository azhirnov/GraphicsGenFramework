...
#	define JNI_ARRAY_TYPE( _type_, _type2_ ) \
		...
		public: \
			...
			\
			explicit \
			JavaArray (Buffer<const _type_> nativeArray) : \
			...
			\
			operator Buffer< const _type_ > () const	{ return Buffer< const _type_ >( ptr(), Count() ); } \
			operator Buffer< _type_ >		  ()		{ return Buffer< _type_ >( ptr(), Count() ); } \
			\
			\
			template <typename B> \
			Buffer< const B >	To () const \
			{ \
				STATIC_ASSERT( sizeof(B) == sizeof(value_t) ); \
				return Buffer< const B >( (B *)ptr(), Count() ); \
			} \
			\
			\
			template <typename B> \
			Buffer< const B >	Cast () const \
			{ \
				ASSERT( Size() % sizeof(B) == 0 ); \
				return Buffer< const B >( (B *) ptr(), Size() / sizeof(B) ); \
			} \
		}
