...
*/
	MeshGenerator::Mesh  MeshGenerator::CreateQuad (const SubSystemsRef ss, const RectF &position, const RectF &texcoord)
	{
		...
		desc.Add( VertexAttribsDesc::AttribIndex(0), EAttribute::Float2, 0 );
		...
		mesh.vertexBuffer->SetAttribs( desc, Bytes<uint>::SizeOf<float4>() );
...
*/
	MeshGenerator::Mesh  MeshGenerator::CreateCube (const SubSystemsRef ss, const float3 &scale, const float3 &bias)
	{
		...
		*/
		...
		VertexAttribsDesc	desc;
		desc.Add( VertexAttribsDesc::AttribIndex(0), EAttribute::Float3, 0 );
		...
		mesh.vertexBuffer->SetAttribs( desc, Bytes<uint>::SizeOf<float3>() );
...
*/
	...
	{
		CHECK_ERR( patchSize == 3 or patchSize == 4 or patchSize == 16, Mesh() );
		CHECK_ERR( numVertices > 0, Mesh() );
		...
		mesh.vertexBuffer->SetAttribs( attr, Bytes<uint>::SizeOf<GridVertex>() );
		mesh.vertexBuffer->SetPrimitive( EPrimitive::Triangle );
...
*/
	...
	{
		CHECK_ERR( patchSize == 3 or patchSize == 4, Mesh() );
		CHECK_ERR( numVertices > 0, Mesh() );
		...
		for (uint z = 0; z < count; ++z)
		{
			...
			{
				float2	t = SinCos( Pi<float> * float(z) / numVertices );
				float2	p = SinCos( Pi<float> * 2.0f * float(i) / numVertices ).yx() * t.x;
...
*/
	MeshGenerator::Mesh  MeshGenerator::CreateCylinder (const SubSystemsRef ss, uint numVertices, float radius, float length, uint patchSize)
	{
		...
		for (uint z = 0; z < numVertices; ++z)
		{
			...
			{
				float2	p = SinCos( Pi<float> * 2.0f * float(i) / circle_vertices ).yx();
