...
real RidgedMulti (real3 point, real frequency, real lacunarity, int noiseQuality, int octaveCount, int seed, const real weights[RIDGED_MAX_OCTAVE]);
void RidgedMulti_CalcSpectralWeights (real lacunarity, real weights[RIDGED_MAX_OCTAVE]);
...
// Default libNoise implementation.
real Voronoi (real3 point, real displacement, real frequency, int seed, bool enableDistance);

// Returns UV coordinate with center in cell center, minimal distance to edge and cell value.
real4 VoronoiEdges (real2 point, real frequency, int seed);

// Returns UV coordinate with center in cell center, minimal distance to center and cell value.
real4 VoronoiUV (real2 point, real frequency, int seed);


// RotatePoint //
// deprecated, use matrices or quaternions.
real3 RotatePoint (real3 point, real3 angle);
...
#define ValueNoise3DV2(p, seed)			ValueNoise3DV3( (int3)( (p), 0 ), (seed) )
...
real Billow (real3 vPoint, real frequency, real lacunarity, real persistence,
				int noiseQuality, int octaveCount, int seed)
{
	real	value	= 0.0;
	real	signal	= 0.0;
	real	pers	= 1.0;
	...
	{
		real3 n = MakeInt32RangeV3( vPoint );
		
		int oseed = (seed + octave) & 0xffffffff;
		signal = GradientCoherentNoise3D( n, oseed, noiseQuality );
		signal = (real)(2.0) * Abs(signal) - (real)(1.0);
		value += signal * pers;
...
// this is variable radiuses cylinders
real Cylinders (real3 vPoint, real frequency)
{
	real distFromCenter = length( vPoint.xz * frequency );
...
real Perlin (real3 vPoint, real frequency, real lacunarity, real persistence,
				int noiseQuality, int octaveCount, int seed)
{
	real	value	= 0.0;
	real	signal	= 0.0;
...
real RidgedMulti (real3 vPoint, real frequency, real lacunarity,
					int noiseQuality, int octaveCount, int seed,
					const real weights[RIDGED_MAX_OCTAVE])
{
	real3 p		= vPoint * frequency;
	real signal = 0.0;
	real value  = 0.0;
	real weight = 1.0;
	real offset = 1.0;
	...
	for (int octave = 0; octave < octaveCount; octave++)
	{
		...
		int oseed = (seed + octave) & 0x7fffffff;
		signal = GradientCoherentNoise3D( n, oseed, noiseQuality );
...
real Select (real3 vPoint, real controlValue, real value0, real value1,
				real lowerBound, real upperBound, real edgeFalloff)
{
	real alpha;
	...
	if ( edgeFalloff > 0.0 )
	{
		...
		if ( controlValue < (lowerBound + edgeFalloff) )
		{
			real lowerCurve = (lowerBound - edgeFalloff);
			real upperCurve = (lowerBound + edgeFalloff);
		...
		if ( controlValue < (upperBound + edgeFalloff) ) {
			real lowerCurve = (upperBound - edgeFalloff);
			real upperCurve = (upperBound + edgeFalloff);
			alpha = SCurve3 ( (controlValue - lowerCurve) / (upperCurve - lowerCurve) );
	...
	{
		if ( controlValue < lowerBound or controlValue > upperBound )
			return value0;
		else
			return value1;
	}
...
real Spheres (real3 vPoint, real frequency)
{
	real3 p = vPoint * frequency;
...
real Voronoi (real3 vPoint, real displacement, real frequency, int seed, bool enableDistance)
{
	vPoint *= frequency;
	...
	{
		real3 pos		= (real3)(	ValueNoise3DV3( cur, seed ),
									ValueNoise3DV3( cur, seed + 1 ),
									ValueNoise3DV3( cur, seed + 2 ) ) + convert_real3(cur);
	...
	return value + (displacement * ValueNoise3DV3( convert_int3(floor(candidate)), 0 ) );
}

real4 VoronoiUV (real2 vPoint, real frequency, int seed)
{
	vPoint *= frequency;

	int2  init	= (int2)( vPoint.x > 0.0 ? (int)(vPoint.x) : (int)(vPoint.x) - 1,
						  vPoint.y > 0.0 ? (int)(vPoint.y) : (int)(vPoint.y) - 1 );
	int2  cur	= (int2)(0);
	
	real minDist = (real)(2147483647.0);
	real2 candidate = (real2)(0);
	
	for (cur.y = init.y - 2; cur.y <= init.y + 2; ++cur.y)
	for (cur.x = init.x - 2; cur.x <= init.x + 2; ++cur.x)
	{
		real2 pos		= Fract2( (real2)(	ValueNoise3DV2( cur, seed ),
											ValueNoise3DV2( cur, seed + 1 ) ) ) + convert_real2(cur);
		real2 distVec	= pos - vPoint;
		real  dist		= dot( distVec, distVec );
	...
	return (real4)( (vPoint - candidate) / frequency,
					minDist,
					ValueNoise3DV2( convert_int2(floor(candidate)), 0 )
				);
}
/*
real4 VoronoiUV3 (real2 vPoint, real frequency, int seed)
{
	vPoint *= frequency;
	...
	real minDist = (real)(2147483647.0);
	real2 candidate = (real2)(0);
	int2 icandidate = (int2)(0);
	
	const int radius = 3;

	for (cur.y = init.y - radius; cur.y <= init.y + radius; ++cur.y)
	for (cur.x = init.x - radius; cur.x <= init.x + radius; ++cur.x)
	{
		real2 offset	= Fract2( (real2)(	ValueNoise3DV2( cur, seed + 1 ),
											ValueNoise3DV2( cur, seed + 2 ) ) );
		real2 pos		= offset * 4.0 + convert_real2(cur);
		real2 distVec	= pos - vPoint;
		real  dist		= dot( distVec, distVec );
	...
	return (real4)( (vPoint - candidate) / frequency,
					minDist,
					ValueNoise3DV2( icandidate, seed )
				);
}
*/
real4 VoronoiEdges (real2 vPoint, real frequency, int seed)
{
	// from http://www.iquilezles.org/www/articles/voronoilines/voronoilines.htm

	vPoint *= frequency;

    int2	n = convert_int2( floor( vPoint ) );
    real2	f = Fract2( vPoint );

	int2	mg;
	real2	mr;
    real	md = (real)(2147483647.0);

    for (int y = -2; y <= 2; ++y)
    for (int x = -2; x <= 2; ++x)
    {
		int2	g = (int2)( x, y );
		real2	o = Fract2( (real2)( ValueNoise3DV2( n + g, seed ),
									 ValueNoise3DV2( n + g, seed + 1 ) ) );
		real2	r = convert_real2( g ) + o - f;
        real	d = dot( r, r );
    ...
    for (int x = -2; x <= 2; ++x)
    {
		...
		{
	        mde = min( mde, dot( (mr + r) * 0.5, normalize( r - mr ) ) );
		}
	}

	real2 candidate = convert_real2(n) + mr + f;
...
real3 RotatePoint (real3 point, real3 angle)
{
	real2	x_sincos = SinCos( angle.x ),
			y_sincos = SinCos( angle.y ),
			z_sincos = SinCos( angle.z );

	real3	row0 = (real3)( y_sincos.s0 * x_sincos.s0 * z_sincos.s0 + y_sincos.s1 * z_sincos.s1,
							x_sincos.s1 * z_sincos.s0,
							y_sincos.s0 * z_sincos.s1 - x_sincos.s0 * z_sincos.s0 );

	real3	row1 = (real3)( y_sincos.s0 * x_sincos.s0 * z_sincos.s1 - y_sincos.s1 * z_sincos.s0,
							x_sincos.s1 * z_sincos.s1,
							-y_sincos.s1 * x_sincos.s0 * z_sincos.s1 - y_sincos.s0 * z_sincos.s0 );

	real3	row2 = (real3)( -y_sincos.s0 * x_sincos.s1,
							x_sincos.s0,
							y_sincos.s1 * x_sincos.s1 );

	return (real3)( dot( row0, point ), dot( row1, point ), dot( row2, point ) );
}
