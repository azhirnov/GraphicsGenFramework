...
namespace Shared
{
	...
	// methods
	public:
		FreeCameraController (const SubSystemsRef ss) :
			IController( ss )
		{
			Reset();
		}
		...
		void Update (Time<double> dt) override
		{
			...
			if ( IsNotZero( rotation ) )
				_camera.RotateFPS( (rotation / _camera.GetZoom()).To< DegreesVec<real,2> >() );

			if ( IsNotZero( motion ) )
				_camera.MoveFPSFree( motion.Normalized() * move_step );
